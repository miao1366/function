...
//     int                 value;  
//     struct {
//     struct _Data  *tqe_next;
//     struct _Data  **tqe_prev;
//     }tailq_entry;
// };  
...
{  
    ...
    // struct tailq_head {
    //     struct _Data *tqh_first;
    //     struct _Data **tqh_last;
    // }head;
    ...
    // do {
    //     (data1)->tailq_entry.tqe_next = NULL;
    //     (data1)->tailq_entry.tqe_prev = (&head)->tqh_last;
    //     *(&head)->tqh_last = (data1);
    //     (&head)->tqh_last = &(data1)->tailq_entry.tqe_next;
    // } while (0)
	/* 3. 在队列末尾插入data2 */  
	...
	/* 5. 在data2之前插入data4 */  
	struct _Data *data4 = (struct _Data *)malloc(sizeof(struct _Data));  
	data4->value = 4;  
	TAILQ_INSERT_BEFORE(data2, data4, tailq_entry);  
	/* 6. 在队列首部插入data5 */  
	struct _Data *data5 = (struct _Data *)malloc(sizeof(struct _Data));  
	data5->value = 5;  
