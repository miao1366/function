...
int post_pack(const char *host, int port, const char *page, int len, const char *content, char *data)
{
	...
	sprintf(post, "%sHost: %s\r\n",post, host);
	sprintf(post, "%s%s", post, HttpsPostHeaders);
	sprintf(post, "%s%s", post, "\r\n\r\n");
...
int ssl_recv(SSL *ssl, char *buff, int size)
{
    ...
    printf("+++++++++++++++++++++++++++++++++++++++++++++++\n");
    char buffer[618718] = {0};
    FILE* pagefile = fopen("mosquitto.html", "wb");
    int fileSize = 0;
    len =0;
	while(fileSize < 618718 && (len = SSL_read(ssl, buffer, 618718)) >0 ) {
        fwrite(buffer, len, 1,pagefile);
        fileSize +=len;
        printf("============================================%d\n", fileSize);
    }
    fclose(pagefile);
...
int Port = 443;
char *Host = "mosquitto.org";
char *Page = "/files/source/mosquitto-1.6.12.tar.gz";
